...
## fourregions_merged_120318
## To work from fourregions_merged_120318 scroll to line #521
...
names(allregions)
functionaltraits <-allregions[,c(19:25,61:143)] 
...
names(functional_groups)
colnames(functional_groups)[1]<-paste("species")
colnames(functional_groups)[2]<-paste("spec_code")
colnames(functional_groups)[3]<-paste("resilience.category")
colnames(functional_groups)[4]<-paste("salinity.tolerance")
colnames(functional_groups)[5]<-paste("depth.category")
colnames(functional_groups)[6]<-paste("size.category")
colnames(functional_groups)[7]<-paste("habitat.category")
colnames(functional_groups)[8]<-paste("trophic_level")
colnames(functional_groups)[9]<-paste("fecundity.min")
colnames(functional_groups)[10]<-paste("fecundity.max")
colnames(functional_groups)[11]<-paste("feeding.guild.category")

#The functional_group database didn't have all of the information in it so I did some work in excel to complete
#missing information.
#Re-writing the functionaldatabase will remove any of the work that was completed in excel!!!
functional_groups<-read.csv("C:/Users/stevensly/Documents/Projects/SPERA/Spreadsheets/Archived/functional_groups.csv",stringsAsFactors = F)
...
#fix column names to read well and have consistant formatting
colnames(alldata)[grep("",colnames(alldata))]<-"species"
colnames(alldata)[4]<-paste("year")
colnames(alldata)[9]<-paste("month")
colnames(alldata)[10]<-paste("day")
colnames(alldata)[18]<-paste("gulf_name")
colnames(alldata)[21]<-paste("phylum")
colnames(alldata)[22]<-paste("subphylum")
colnames(alldata)[23]<-paste("class")
colnames(alldata)[24]<-paste("order")
colnames(alldata)[25]<-paste("family")
colnames(alldata)[26]<-paste("genus")
colnames(alldata)[27]<-paste("date")
colnames(alldata)[28]<-paste("format")
colnames(alldata)[29]<-paste("location")
colnames(alldata)[30]<-paste("longitude")
colnames(alldata)[31]<-paste("latitude")
colnames(alldata)[32]<-paste("taxon")
colnames(alldata)[33]<-paste("individual.count")
colnames(alldata)[34]<-paste("weight_kg")
colnames(alldata)[35]<-paste("presence")
colnames(alldata)[36]<-paste("biomass")
colnames(alldata)[37]<-paste("density")
colnames(alldata)[38]<-paste("coverage")
colnames(alldata)[39]<-paste("sampling.method")
colnames(alldata)[40]<-paste("collection")
colnames(alldata)[41]<-paste("owner.institution")
colnames(alldata)[45]<-paste("strat_type")
colnames(alldata)[45]<-paste("mission")
colnames(alldata)[46]<-paste("name")
colnames(alldata)[47]<-paste("season")
colnames(alldata)[48]<-paste("datetime")
colnames(alldata)[49]<-paste("distance")
colnames(alldata)[50]<-paste("surf_temp")
colnames(alldata)[51]<-paste("bott_temp")
colnames(alldata)[52]<-paste("bott_salinity")
colnames(alldata)[53]<-paste("e_longitude")
colnames(alldata)[54]<-paste("e_latitide")
colnames(alldata)[55]<-paste("dmin")
colnames(alldata)[56]<-paste("dmax")
colnames(alldata)[57]<-paste("totno")
colnames(alldata)[5]<-paste("totwgt")
colnames(alldata)[60]<-paste("phylum")
colnames(alldata)[61]<-paste("subphylum")
colnames(alldata)[62]<-paste("class")
colnames(alldata)[63]<-paste("order")
colnames(alldata)[64]<-paste("family")
colnames(alldata)[65]<-paste("genus")
...
#write.csv(alldata, file='C:/Users/stevensly/Documents/Projects/SPERA/Spreadsheets/Archived/fourregions_merged_120318.csv')
fourregions_merged<-read.csv("C:/Users/stevensly/Documents/Projects/SPERA/Spreadsheets/Archived/fourregions_merged_120318.csv",stringsAsFactors = F)
...
pointdata_allregions <- subdata[!subdata$species%in%names(which(table(subdata$species)>Precent_1_stations)),c("longitude","latitude","year_final")]
