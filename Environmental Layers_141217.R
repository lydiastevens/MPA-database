...
#There appears to be 1969 layers
future_layers$layer_code[1:1969]
future_layers$current_layer_code[1:1969]


###Future Salinity###
salinity_future <- load_layers(c("BO_A1B_2100_salinity")) 
ne.atlantic.ext <- extent(-70.9, -47.3, 41.9, 58.6)
salinity_future.crop <- crop(bott_salinity_future, ne.atlantic.ext) 
my.colors = colorRampPalette(c("#5E85B8","#EDF0C0","#C13127")) 
plot(salinity_future.crop,col=my.colors(1000),axes=FALSE, box=FALSE)
title(main = "Future Salinity Year 2100")
map("worldHires", xlim=Long.lim, ylim=Lat.lim, col="white", fill=TRUE, resolution=0,interior = FALSE,lty=0,add=T)
map("worldHires", xlim=Long.lim, ylim=Lat.lim, col="black", fill=TRUE, resolution=0,interior = FALSE,lty=0,add=T)


future_layers[future_layers$layer_code=="WC_tmax3_mr85_2050",]
